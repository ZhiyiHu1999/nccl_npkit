...
ncclResult_t ncclTopoPrintGraph(struct ncclTopoSystem* system, struct ncclTopoGraph* graph) {
  INFO(NCCL_ENV, "Pattern %d, crossNic %d, nChannels %d, bw %f/%f, type %s/%s, sameChannels %d", graph->pattern, graph->crossNic, graph->nChannels, graph->bwIntra, graph->bwInter, topoPathTypeStr[graph->typeIntra], topoPathTypeStr[graph->typeInter], graph->sameChannels);  // NCCL_GRAPH
  int ngpus = system->nodes[GPU].count;
  ...
  for (int c=0; c<graph->nChannels; c++) {
    ...
    INFO(NCCL_ENV, "%s", line);  // NCCL_GRAPH
  }
  return ncclSuccess;
}
